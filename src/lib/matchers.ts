...
import { YnabDefaultError } from "../models/errors/ynabdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | YnabDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | YnabDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new YnabDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new YnabDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
